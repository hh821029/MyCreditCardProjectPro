...
def run_analytics(
    ...
    if os.path.exists(merchants_config_path):
        ...
        # 補上次分類對照
        if 'sub_category' in df_merchants.columns:
            sub_category_map = dict(zip(df_merchants['merchant_display'], df_merchants['sub_category']))
            df_raw['sub_category'] = df_raw['merchant_display'].map(sub_category_map).fillna('')
        else:
            df_raw['sub_category'] = ''
    else:
    ...
    logger.info(f"✅ 成功載入 {len(df_raw)} 筆交易資料，並已動態掛載分類與次分類。")

    # [篩選消費類別]
    if categories is not None:
        df_raw = df_raw[df_raw['category'].isin(categories)]
        logger.info(f"🧹 已依據消費類別進行篩選，剩下 {len(df_raw)} 筆交易資料。")

    # [篩選消費次類別]
    if sub_categories is not None:
        df_raw = df_raw[df_raw['sub_category'].isin(sub_categories)]
        logger.info(f"🧹 已依據消費次類別進行篩選，剩下 {len(df_raw)} 筆交易資料。")
